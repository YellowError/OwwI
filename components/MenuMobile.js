    ...
    buttons.forEach((button) => {
      ...
      if (button.position === "left") {
        ...
        if (button.id) {
          ...
          left.push(
            <CreateButton
              cible={`${button.cible}/${button.id}`}
              key={buttons.findIndex((but) => but.title == button.title)}
              style={""}
            >
              <SvgsMenu choice={button.title} />
            </CreateButton>
          );
        } else {
  ...
  return (
    <div className="bg-white grid grid-cols-12 w-full p-2">
      <div className="col-span-5 flex justify-around">
        ...
        {leftButtons.length > 0 ? leftButtons.map((element) => element) : ""}
      </div>
      <div className="col-span-2">
        <MainButton
          mainButton
          title={mainButton.title}
          logic={mainButton.logic}
